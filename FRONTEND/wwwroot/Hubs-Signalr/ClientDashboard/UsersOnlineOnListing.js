...
var listingId = url.replace(/\/\s*$/, "").split('/').pop();

// Shafi: Create connection
var connection = new signalR.HubConnectionBuilder().withUrl("/usersOnlineOnListingHub?listingId=" + listingId + "").build();
// End:

// Shafi: When connection is on executive this
connection.on("UsersOnline", function (count) {

    $("#liveUsers").html(count + " Users");
